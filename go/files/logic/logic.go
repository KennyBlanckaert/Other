...
func initialize(object models.Copy) {
	if object.Share.Path != "" && object.Share.Name != "" {
		if runtime.GOOS == "windows" {
			exec.Command("cmd.exe", "/C", "net", "use", object.Share.Path, "/delete", "/yes").Run()
			exec.Command("cmd.exe", "/C", "net", "use", object.Share.Path, object.Share.Name, "/yes").Run()
			exec.Command("cmd.exe", "/C", "mkdir", object.Destination).Run()
		}
		if runtime.GOOS == "linux" {
			exec.Command("umount", object.Share.Path).Run()
			exec.Command("mkdir", "-p", object.Share.Path).Run()
			exec.Command("mkdir", "-p", object.Destination).Run()

			options := fmt.Sprintf("username=%s,password=%s", object.Credential.User, object.Credential.Password)
			exec.Command("mount", "-t", "cifs", object.Share.Name, object.Share.Path, "-o", options).Run()
		}
	}
